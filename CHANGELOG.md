...
- `attempts` (zest-server 1.2.2, Bridge 3.2): `zest-context` carries `context.attempts` (`allowed`, `used`, `submissions`, `canRetry`, `source`), from the assignment's attempt limit when the Developer Key sends `$Canvas.assignment.allowedAttempts` and `$Canvas.assignment.submission.studentAttempts`; `Zest.getAttempts()` returns it. The bridge's `clientSubmitId` seed includes the number of submissions already made, so identical results on a new attempt are a new submission rather than a retry.
- `state.reset`: the student wrapper offers "Start a new attempt" when `canRetry` is true and the student has submitted. It posts `zest-reset` to the content; the bridge drops its local copy and pending syncs and answers `zest-reset-result`; the wrapper deletes the server state for that assignment and reloads the content. Packages need no retry logic; one that locks itself after submitting should consult `canRetry`.
